...
// For each node and universe, calculates the amount of code within the node
// that is contained in the universe.
//
// At the lowest level in the tree, the concept of code shared between universes
// becomes a bit blurry. For example, a method `foo` may have a code size of 300
// bytes in universe A and only 120 bytes in universe B. Now, what amount of
// code is shared? The correct answer is that we can't tell – the method may
// have a completely different implementation, or perhaps it's the same code but
// the GraalVM analysis had a bit more information about type flows and was able
// to simplify the code.
//
// It would make no sense to treat the implementation of methods as entirely
// different (this would make the visualization useless), but it also doesn't
// make sense to treat the unioned code size as being in all universes – after
// all, a smaller universe should be reported to be smaller.
//
// What we do here is a "share-as-much-as-is-reasonable" approach: Let's say
// there's a method with the following code sizes in three universes:
//
// - 20 bytes in universe A
// - 50 bytes in universe B
// - 80 bytes in universe C
//
// We treat the minimum size (20 bytes) as being shared among all universes. We
// then repeat this process for the universes that still contain more code with
// the remaining code sizes. Finally, we end up with this result:
//
...
function computeExclusiveSizesRec(
    ...
    if (mergedNode.children.length === 0) {
        ...
        mergedNode.children.forEach((child) => {
